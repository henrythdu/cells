...
- cpp: scip-clang emits header→source definition-use edges (symbol def lives in
  the header); headers aren't importers in the cells model, so the oracle-side
  scoping drops from-ext headers — source→header `#include` edges stay.

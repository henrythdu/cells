...
import type { ImportEdge, SourceFile, Importer } from './imports.js';
...
export async function collectImportEdges(baseDir = '.'): Promise<{
  ...
  const results = await Promise.all(selected.map((imp) => imp.extract({ codeDirs: dirs, files, ownership, baseDir }).catch(() => [])));
  const edges: ImportEdge[] = [];

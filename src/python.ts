...
function resolveEdges(
	...
	if (desc.dots === 0) {
		...
		base = desc.module
			? [...targetPkg, ...desc.module.split(".")].join(".")
			: targetPkg.join(".");
	}
	const candidates = [
		base,
		...desc.names.map((n) => (base ? `${base}.${n}` : n)),
	];
...
export const pythonImporter = createTreeSitterImporter({
	...
	extractEdges: (root, sourcePath, _importerModule, moduleToFile) =>
		extractImports(root).flatMap((desc) =>
			resolveEdges(desc, sourcePath, moduleToFile),
		),
});

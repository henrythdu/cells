...
/** Does `path` match the ignore patterns (gitignore-style globs, `**` supported; basename
 *  patterns match at any depth via matchBase; a trailing `/` marks a directory → matches
 *  the whole tree under it, like gitignore's `dir/`)? Ordered gitignore semantics: patterns
 *  evaluate top-to-bottom and the LAST match wins — a `!`-prefixed pattern re-includes
 *  what an earlier pattern ignored. (Limitation, same as gitignore: a file re-included
 *  under an ignored DIRECTORY stays ignored — no parent-dir tracking.)
...
export function isIgnored(path: string, patterns: string[]): boolean {
  ...
  for (const raw of patterns) {
    ...
    const p = negate ? raw.slice(1) : raw;
    const pattern = p.endsWith('/') ? `**/${p}**` : p; // `dist/` → `**/dist/**` — gitignore dir semantics, any depth
    if (minimatch(path, pattern, { matchBase: true, dot: pattern.includes('**') })) ignored = !negate;
  }
  return ignored;
}
